...
*/
#ifndef MultiTimer2_h
#define MultiTimer2_h

#include <TimerTask.h>
#include <MsTimer2.h>

namespace MultiTimer2
{
	void AddTask(void (*FunctionToCall)(void), unsigned long msec, unsigned long NrOfTimes);
	void start();
	void stop();

	void AddTask(volatile TimerTask *theTask);
	void Interrupt();

	volatile extern TimerTask *TaskList;
	volatile extern int IsRunning;
	void _overflow();
};
